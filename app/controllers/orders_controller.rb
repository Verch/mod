    ...
    respond_to do |format|
      if @current_group && @current_group.admin_flag  
        case params[:temp_order_id]
          ...
          when "2"
            @orders = Order.where("reserv_flag = ?", false).order("created_at DESC")
          when "3"
            @orders = Order.where("reserv_flag = ?", true).order("name ASC")
          when "4"
            @orders = Order.where("reserv_flag = ?", true).order("created_at DESC")
          else
            @orders = Order.where("reserv_flag = ?", false).order("name ASC")  
        end
      else
